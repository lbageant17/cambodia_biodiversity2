...
tempname shapley
postfile `shapley' str40(Outcome) str40(Predictor) pred effort effort2 hhsize depshare maxeduc amenities livelihood city_distance total_r2 using "$temp/shapley_$date",replace

* Set up grouped regression decomposition output file
tempname shapleyg
postfile `shapleyg' str40(Outcome) str40(Predictor) pred effort demog assets city_distance total_r2 using "$temp/shapley_grouped_$date",replace
...
use "$processed/hh_level_stata", clear
	...
	postclose `shapleyg'
	use "$temp/shapley_grouped_$date", clear
