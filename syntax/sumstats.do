...
global diversity biom_species catch_species consumption_species sold_species h_biom_cfr_level h_catch h_cons h_sold
global hh hhsize depshare maxeduc index1 index2 buff chicken cow duck pig land incfish incfarm incwage incskill
global cfr city_distance cat1 cat2 cat3 cat4
	
estpost tabstat $diversity $hh $cfr, statistics (p50 mean sd min max ) columns(statistics) casewise
esttab using "$output/$date/sumstats", cells ("p50(fmt(2)) mean(fmt(2)) sd(fmt(2)) min(fmt(2)) max(fmt(2)) ") 		///
	nostar replace label unstack nonumber ///
	title("Summary statistics of key variables") csv

	
** Look at missing values
foreach var of varlist $cfr $hh $diversity {
	qui: count if `var' != .
	di _newline "`var'"
	di r(N)
	}
