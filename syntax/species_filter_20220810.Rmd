...
---
title: "Cambodia biodiversity paper notes -- Log rank abundance plots--August 10, 2022"
output: 
  pdf_document:
...
# Bimonitoring
logra_biom <- log_ra %>% 
  filter(type == "Biomonitoring") %>% 
  ggplot(aes(x = rank, y = weight)) + 
  geom_line() +
  geom_point(size=2, alpha=0.3, color = "#006666") +
  geom_text(aes(label = ifelse(rank <= 2, sname,'')),
            hjust = 0, vjust = -0.5,
            size = 2) +
  geom_text(aes(label = ifelse(rank == 3, sname,'')),
            hjust = 0, vjust = 0.5,
            size = 2) +
  geom_text(aes(label = ifelse(rank == 4 | rank == 5, sname,'')),
            hjust = 0, vjust = 0,
            size = 2) +
  ...
  geom_text(aes(label = ifelse(rank == 7 , sname,'')),
            ...
            size = 2) +
  simple +
  ggtitle("1. Log Rank Abundance--Biomonitoring") +
  ylab("log1p(weight)") +
  ylim(0, 12.2)
...
logra_catch <- log_ra %>% 
  ...
  geom_text(aes(label = ifelse(rank == 7, sname,'')),
            ...
            size = 2) +
  simple +
  ggtitle("2. Log Rank Abundance--Catch")+
  ylab("log1p(weight)") +
  ylim(0, 12.2)
...
logra_cons <- log_ra %>% 
  ...
  geom_text(aes(label = ifelse(rank == 6, sname,'')),
            ...
            size = 2) +
  simple +
  ggtitle("3. Log Rank Abundance--Consumption")+
  ylab("log1p(weight)") +
  ylim(0, 12.2)



## RANK ABUNDANCE (NOT LOGGED)
# Bimonitoring
ra_biom <- ra %>% 
  filter(type == "Biomonitoring") %>% 
  ggplot(aes(x = rank, y = weight)) + 
  geom_line() +
  geom_point(size=2, alpha=0.3, color = "#CC00CC") +
  geom_text(aes(label = ifelse(rank <= 2, sname,'')),
            hjust = 0, vjust = -0.5,
            size = 2) +
  geom_text(aes(label = ifelse(rank == 3, sname,'')),
  ...
  geom_text(aes(label = ifelse(rank == 7 , sname,'')),
            ...
            size = 2) +
  simple +
  ggtitle("4. Rank Abundance--Biomonitoring") 
...
ra_catch <- ra %>% 
  ...
  geom_text(aes(label = ifelse(rank == 7, sname,'')),
            ...
            size = 2) +
  simple +
  ggtitle("5. Rank Abundance--Catch")
...
ra_cons <- ra %>% 
  ...
  geom_text(aes(label = ifelse(rank == 6, sname,'')),
            ...
            size = 2) +
  simple +
  ggtitle("6. Rank Abundance--Consumption")
  
  
```

I calculated log rank abundance (figs 1-3) and rank abundance (figs 4-6) at the system level (biomonitoring data), catch level and consumption level. 

I plotted only the top 100 species to make it more readable. There are approx 120 species total.

I can do a Spearman rank correlation test to see how correlated (or not) the rank abundances are across biomonitoring, catch and consumption for all ~120 species. I would expect them to be pretty correlated, despite the differences we see below.

I labeled the top 7 species in each plot and they are summarized for Figures 1-3 in table below. The highlighted items are things that are only present in a single column (i.e. ranked highly in one category but not in the others).

![](../images/rank_abundance_table.png){width=300px}
...
The figures below are not logged and the y-axes are scaled differently
\
\
