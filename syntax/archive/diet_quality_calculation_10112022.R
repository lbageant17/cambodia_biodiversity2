...
# Set RDA threshold (percentage of RDA met for a given nutrient)
...
#------------------------------------------------------------------------------#   
  ...
  #### ----- Calculate proportional abundance by species from CCM data ------ ####   
    ...
    # calculate system proportional abundance by species
    pa_sys_ccm <- c %>% 
      select(scode_ccm, catch_iweight) %>% 
    ...
    pa_sys_ccm %>% filter(pa == 0) # no zero values
  ...
  #### ----- Calculate how many RDAs are met by 100g of a given fish portfolio using RDA threshold set above ------ #### 
    ...
    rda_sys_ccm <- portfolio_nutrient_concentration %>% 
      ...
      mutate(pr_rda = if_else(pr_g > protein_g, 1, 0),
             ...
             vita_rda = if_else(vita_mcg > vitamina_mcg, 1, 0),
             o3_rda = if_else(o3_mg > omega3_mg, 1, 0),
             # total number of RDAs met by 100g of system CCM portfolio at the RDA threshold set above
             rda_sys_ccm = pr_rda + fe_rda + zn_rda + ca_rda + vita_rda + o3_rda) %>% 
      select(rda_sys_ccm)
