...
tl_system <- b %>% 
  left_join(ccm_traits_specieslevel, by = "scode_biom") %>%  #nrow() 7842
  mutate(abundance_weighted_tl = tl*rel_abundance) %>% 
  summarise(mbs_system = mean(abundance_weighted_tl, na.rm = TRUE)) # 1.45
...
### CFR level mean total length
  ...
  pa_cfr <- b %>%
    ...
    mutate(total = total_biomass_biom,
           pa_cfr = biomass/total)
...
### HOUSEHOLD LEVEL mean total length--catch, consumption, sold
  ...
  pa_catch <- c %>% 
    ...
    summarise(biomass = sum(catch_iweight)) %>% 
    mutate(total = pa_catch = biomass/total_biomass_catch)
