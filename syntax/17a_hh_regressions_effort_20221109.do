...
* These regressions will be calculated with robust SEs and output
	...
	* interact market access
	...
	outreg2 [base effort hh mkt mktx] using "$output/$date/tables/hh_regs_effort/`metric'_`tablename'", replace excel label ///
...
export excel using "$output/$date/tables/betas_hh.xlsx", sheet("HH models") sheetmodify firstrow(variables)


postclose `wald'
use "$temp/wald_test_hh", clear
export excel using "$output/$date/tables/wald_test_hh.xlsx", sheet("HH models") sheetmodify firstrow(variables)
