...
#--------- Box plot at CFR level ---------------------------------------------*/

# partition off CFR level to add back later 
add <- cfr_biodiv_long %>%
  filter(type == "cfr")

# bring in CFR IDs where they are missing
cfr_species <- cfr_biodiv_long %>%
  select(-cfrid) %>% 
  filter(type != "cfr") %>% 
  left_join(cfrid_to_hhid, by = "hhid") %>% 
  rbind(add) %>% # adding back CFR level data
  group_by(cfrid, type) %>% 
  summarise(species = mean(species))

# create a data fram of CFR-level mean number of species
cfr <- cfr_species %>% 
  filter(type == "cfr") %>% 
  rename(cfr_species = species) %>% 
  select(-type) 

# merge CFR-level mean number of species file with main data frame and plot.
cfr_species %>% 
  left_join(cfr, by = "cfrid") %>% 
  ggplot(aes(x = type, y = species, fill = type)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(aes(color = cfr_species, size = cfr_species), alpha = 0.7) +
  scale_fill_viridis(discrete = TRUE, alpha = 0.7) +
  scale_x_discrete(limits = order, labels=c("CFR","Catch","Consumed", "Sold")) +
  theme_bw() +
  ylab("Mean species richness") +
  labs(title = "Mean species richness by portfolio type (CFR level)")
